...
<Callout type="info">
  In the Uniswap v3 PositionsNFT contract, the `decreaseLiquidity` function has
  a special modifier called `isAuthorizedForToken`. This modifier acts like a
  gatekeeper, ensuring that only authorized senders can perform this action.
  Uniswap v3 has these checks built into its system, so there's no need for
...
- **Function:** `	claim(uint256,address,address[],uint256[],bytes32[])`
  ...
  | `tokens`      | equal to a subset of (`rETH2`,`SWISE`) |
  | `amounts`     | -                                      |
  | `merkleProof` | -                                      |

### If WETH is involved, the following functions are also whitelisted:
