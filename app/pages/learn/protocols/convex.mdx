...
Deposit `TARGET_CVX_DEPOSIT_TOKEN` tokens to the corresponding Convex pool and receive `TARGET_CVX_DEPOSIT_TOKEN` tokens in exchange.<br/>
...
Deposit `TARGET_CVX_DEPOSIT_TOKEN` tokens to the corresponding Convex pool and receive `TARGET_CVX_DEPOSIT_TOKEN` tokens in exchange (it is called when MAX amount is deposited).<br/>
The `TARGET_CVX_DEPOSIT_TOKEN` can be staked using the `stake` parameter and receive `TARGET_REWARDER` tokens in exchange.
...
- **Function:** `deposit(uint256,bool,address)`
  ...
  | `amount`       | -                              |
  | `lock`         | -                              |
  | `stakeAddress` | equal to (`0x` or `stkCvxCrv`) |
