...
<Callout type="info">
  `TARGET` below refers to the `Pool` structure of each element given in
  `targets`.<br/>
  `Pool` structure:<br/>
  &nbsp;&nbsp;&nbsp;&nbsp;\{<br/>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'name': `bpt` symbol,<br/>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'bpt': `bpt` address,<br/>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'id': Aura pool id<br/>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'rewarder': `auraBaseRewardPool` address<br/>
  &nbsp;&nbsp;&nbsp;&nbsp;\}
</Callout>
...
Approve `Booster` to spend `TARGET-bpt` tokens on behalf of the avatar
...
Deposit `TARGET-bpt` tokens to the corresponding Aura pool and receive `TARGET-rewarder` tokens in exchange
...
- **Function:** `deposit(uint256,uint256,bool)`
  ...
  | Parameter | Constraint           |
  | :-------- | :------------------- |
  | `pid`     | equal to `TARGET-id` |
  | `amount`  | -                    |
  | `stake`   | -                    |

#### `RewardPoolDepositWrapper · depositSingle`

Deposit `TOKEN` tokens to the corresponding Aura pool and receive `TARGET-rewarder` tokens in exchange
...
- **Function:** `depositSingle(address,address,uint256,bytes32,(address[],uint256[],bytes,bool))`
  ...
  | Parameter                     | Constraint                 |
  | :---------------------------- | :------------------------- |
  | `rewardPoolAddress`           | equal to `TARGET-rewarder` |
...
#### `TARGET-rewarder · withdrawAndUnwrap`

Redeem `TARGET-rewarder` tokens and withdraw and unwrap `TARGET-bpt` tokens

- **Contract:** `TARGET-rewarder`
...
#### `TARGET-rewarder · getReward(address,bool)`
...
- **Function:** `deposit(uint256,uint256,bool,address)`
  ...
  | `minOut`       | -                                                             |
  | `lock`         | -                                                             |
  | `stakeAddress` | equal to (`0x` or `AuraBalBaseRewardPool` or `AuraBalStaker`) |
