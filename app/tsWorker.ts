...
interface CustomTSWebWorkerFactory {
  (
    TSWorkerClass: languages.typescript.TypeScriptWorker,
    tsc: typeof ts,
...
const worker: CustomTSWebWorkerFactory = (
  TypeScriptWorker: languages.typescript.TypeScriptWorker,
  ts,
  libFileMap
) => {
  // @ts-ignore
  ...
  const extension = (
    !!params.get("useJavaScript") ? "js" : params.get("filetype") || "ts"
  ) as any
  ...
  return class MonacoTSWorker extends TypeScriptWorker {
    ...
    updateTwoslashInfoIfNeeded(): void {
      ...
      // Do the work
      const splits = splitTwoslashCodeInfoFiles(
        modelValue,
        this.mainFile,
        "file:///"
      )
      ...
      this.additionalTwoslashFilenames = twoslashResults
        .map((f) => f.file)
        .filter((f) => f !== this.mainFile)
      this.twolashFilesModelString = modelValue

      if (convertedToMultiFile) {
        console.log(
          "Switched playground to use multiple files: ",
          this.additionalTwoslashFilenames
        )
      }
    }
    ...
    repositionInTwoslash(fileName: string, position: number) {
      ...
      const thisFile = this.twoslashFiles.find(
        (r) => r.startIndex < position && position <= r.endIndex
      )
    ...
    override async getSemanticDiagnostics(fileName: string) {
      return this._getDiagsWrapper(
        super.getSemanticDiagnostics.bind(this),
        fileName
      )
    }
    ...
    override async getSyntacticDiagnostics(fileName: string) {
      return this._getDiagsWrapper(
        super.getSyntacticDiagnostics.bind(this),
        fileName
      )
    }
    ...
    override async getCompilerOptionsDiagnostics(fileName: string) {
      return this._getDiagsWrapper(
        super.getCompilerOptionsDiagnostics.bind(this),
        fileName
      )
    }
    ...
    override async getSuggestionDiagnostics(fileName: string) {
      return this._getDiagsWrapper(
        super.getSuggestionDiagnostics.bind(this),
        fileName
      )
    }
    ...
    override async getQuickInfoAtPosition(fileName: string, position: number) {
      ...
      const pos = await this._overrideFileNamePos(
        super.getQuickInfoAtPosition.bind(this),
        fileName,
        position,
        undefined,
        empty,
        (result: QuickInfo | undefined, twoslashFile) => {
    ...
    override async getCompletionsAtPosition(
      fileName: string,
      position: number
    ) {
      const empty = Promise.resolve({
      ...
      const completions = await this._overrideFileNamePos(
        super.getCompletionsAtPosition.bind(this),
        fileName,
        position,
        undefined,
        empty,
    ...
    override async getCompletionEntryDetails(
      ...
      return this._overrideFileNamePos(
        super.getCompletionEntryDetails.bind(this),
        fileName,
        position,
        entry,
        empty,
    ...
    override async getOccurrencesAtPosition(
      fileName: string,
      position: number
    ) {
      const empty = Promise.resolve([])
      return this._overrideFileNamePos(
        super.getOccurrencesAtPosition.bind(this),
        fileName,
        position,
        undefined,
        empty,
        (result) => {
          if (result) {
            result.forEach((re) => {
              const twoslash = this.twoslashFiles.find(
                (f) => f.file === re.fileName
              )
    ...
    override async getDefinitionAtPosition(fileName: string, position: number) {
      const empty = Promise.resolve([])
      return this._overrideFileNamePos(
        super.getDefinitionAtPosition.bind(this),
        fileName,
        position,
        undefined,
        empty,
    ...
    override async getReferencesAtPosition(fileName: string, position: number) {
      const empty = Promise.resolve([])
      return this._overrideFileNamePos(
        super.getReferencesAtPosition.bind(this),
        fileName,
        position,
        undefined,
        empty,
    ...
    override async getNavigationBarItems(fileName: string) {
      const empty = Promise.resolve([])
      return this._overrideFileNamePos(
        super.getNavigationBarItems.bind(this),
        fileName,
        -1,
        undefined,
        empty,
    ...
    async _overrideFileNamePos<
      T extends (fileName: string, position: number, other: any) => any
    >(
      fnc: T,
      fileName: string,
      position: number,
      ...
      editFunc: (
        ...
        twoslash: TwoSlashFiles[0] | undefined
      ) => any
      ...
      editFunc(
        result,
        this.twoslashFiles.find((f) => f.file === tsFileName)
      )
      return result
    }
    ...
    async _getDiagsWrapper(
      getDiagnostics: (a: string) => Promise<Diagnostic[]>,
      fileName: string
    ) {
      ...
      for (const f of this.twoslashFiles) {
        ...
        d.forEach((diag) => {
          if (diag && diag.start) diag.start += f.startIndex
        })
...
const splitTwoslashCodeInfoFiles = (
  code: string,
