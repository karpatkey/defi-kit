...
export const allowReceive: TransactionsHandler = async (query) => {
  const {
    mod: { address, chain },
    role,
    protocol,
...
export const registerAllowReceive = (
  ...
  registry.registerPath({
    method: "get",
    path: `/${chainPrefix}:{mod}/{role}/allow/${protocol}/receive`,
    summary: `Transactions for granting permissions to receive the specified tokens`,
    tags: [`${protocol} allow`],
    request: {
...
export const receivePermissions: PermissionsHandler = async (query) => {
  const permissions = await queryActionPermissionSet({
    action: "receive",
...
export const registerReceivePermissions = (
  registry: OpenAPIRegistry,
  chainPrefix: ChainPrefix,
  protocol: string
) => {
  ...
  registry.registerPath({
    method: "get",
    path: `/permissions/${chainPrefix}/${protocol}/receive`,
    summary: `Receive the specified tokens on ${protocol}`,
    ...
    responses: {
      200: {
        description: "Permissions for receiving the specified tokens",
        content: {
          "application/json": {
