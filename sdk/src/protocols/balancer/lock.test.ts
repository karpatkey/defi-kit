...
describe("balancer", () => {
  describe("lock", () => {
      ...
      await expect(
        testKit.eth.balancer.vault.joinPool(
          ...
          avatar._address,
          {
            ...
            userData: "",
    ...
    it("lock and claim", async () => {
      let unlock_timestamp = Math.floor(new Date().getTime() / 1000) + 30 * 24 * 60 * 60
      await expect(
        testKit.eth.usdc.attach(B_80BAL_20WETH).approve(
          contracts.mainnet.balancer.vebal,
          parseEther("200")
        )
      ).not.toRevert()
      ...
      // Test that the transaction goes through the roles
      await expect(
        testKit.eth.balancer.vebal.increase_amount(
          parseEther("200")
        )
      ).toBeAllowed()
      ...
      // Test that the transaction goes through the roles
      await expect(
        testKit.eth.balancer.vebal.increase_unlock_time(
          unlock_timestamp
        )
      ).toBeAllowed()
      ...
      // Test that the transaction goes through the roles
      await expect(
        testKit.eth.balancer.vebal.withdraw()
      ).toBeAllowed()

      // Claim only with avatar as user
      await expect(
        testKit.eth.balancer.fee_distributor.claimTokens(
          avatar._address,
          [bb_a_USD_v1, bb_a_USD_v2, bb_a_USD_v3, BAL, contracts.mainnet.usdc]
        )
      ).not.toRevert()
